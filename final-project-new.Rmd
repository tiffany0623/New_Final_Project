...
sum(is.na(Life_Exp))
```
We have excluded the missing value during the previous data cleaning process, so the missing value now is zero.
...
head(Life_Exp_model)
```
Compared with the tables before, we can find the cleam_names() function make the variables name changed with lower-case letter. Besides, the space which separate the words becomes the underscore “_“. Since this function standardizes all the variable names into the same clear format, it will be more convenient and easier for the latter understanding and data processing.
...
Here we split the data with 80% train and 20% test and stratify on the life_expectancy.
...
There are 1318 observations in the training set, and there are 331 observations in the test set.
...
* Exclude the variable "country"

Since our model fitting mainly focused on the life expectancy, status, and other numerical variables, we decide to exclude the variable country in the training and test set before the model fitting.
```{r}
Life_train %>%
  select(-country)
Life_test %>%
...
We fold the training data with v-fold cross-validation, with v=5.
```{r}
...
### Create the recipe
Using the training data, we create a recipe predicting the outcome variable, life-expectancy, with all other predictor variables. 

Steps for my recipe:
* dummy code all nominal predictors
* create interactions between infant_deaths and under_five_deaths, thinness_1_19_years and thinness_5_9_years, between gdp and percentage_expenditure
...
bt_grid <- grid_regular(trees(range = c(10,200)), tree_depth(range = c(1,10)), levels = 10)
```
